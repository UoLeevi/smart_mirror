...
static void *recv_weather(
    uo_ipcmsg ipcmsg,
    uo_cb *recv_weather_cb)
{
    weather_widget *widget = uo_cb_stack_pop(recv_weather_cb);
    int ipcsockfd = (uintptr_t)uo_cb_stack_pop(recv_weather_cb);
    uo_ipc_disconnect(ipcsockfd);

    char *weather = uo_ipcmsg_get_payload(ipcmsg);
	...
	void *arg)
{
    ...
    while (!widget->is_close_requested)
    {
        ...
        int ipcsockfd = uo_ipc_connect("localhost", "12001");

        uo_cb *recv_weather_cb = uo_cb_create(UO_CB_OPT_DESTROY);
        uo_cb_append(recv_weather_cb, (void *(*)(void *, uo_cb *))recv_weather);
        uo_cb_stack_push(recv_weather_cb, (void *)(uintptr_t)ipcsockfd);
        uo_cb_stack_push(recv_weather_cb, widget);

        uo_ipc_sendmsg(ipcsockfd, buf, cmd_len, recv_weather_cb);
